...
const EditSubject = () => {
  ...
  const { subjectDetails, loading, fetchSubjectDetails, editSubject } = useSubjects();
  const {
    register,
    handleSubmit,
    reset,
  ...
  useEffect(() => {
    if (subjectDetails) reset(subjectDetails);
  ...
  if (loading && !subjectDetails) return <Skeleton className="h-72 rounded-2xl" />;

  return (
    <div className="flex flex-col gap-6">
      <AdminTopbar title="Edit Subject" subtitle={`Editing record #${id}`} />
