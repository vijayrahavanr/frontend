...
const Settings = () => {
  ...
  return (
    <div className="flex flex-col gap-6">
      <FacultyTopbar title="Settings" subtitle="Manage your preferences and account" />
      ...
      <Section title="Notifications" spacing="sm">
        <div className="rounded-2xl border border-slate-200 bg-white px-5 dark:border-slate-700 dark:bg-surface-dark-elevated">
          ...
          <SettingsRow
            ...
            description="Get notified when a student applies for leave"
            control={<Switch checked={notifPrefs.leaveAlerts} onChange={() => toggle('leaveAlerts')} />}
          />
          <Divider />
          <SettingsRow
            icon={<FiBell size={16} />}
            title="Attendance reminders"
            description="Get reminded to submit attendance"
            control={
              <Switch
                checked={notifPrefs.attendanceReminders}
                onChange={() => toggle('attendanceReminders')}
              />
            }
          />
        </div>
