...
/**
 * Profile edit form — dispatches studentSlice's updateProfile and
 * uploadProfilePhoto thunks via useStudent. Shows a local preview of
 * a selected photo before it's uploaded.
 */
const EditProfile = () => {
  const navigate = useNavigate();
  const { profile, loading, fetchProfile, editProfile, uploadPhoto } = useStudent();
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm({ defaultValues: { name: '', email: '', phone: '', address: '' } });
  ...
  return (
    <div className="flex flex-col gap-6">
      <StudentTopbar title="Edit Profile" subtitle="Update your personal information" />
      ...
      <form onSubmit={handleSubmit(onSubmit)} noValidate className="flex flex-col gap-6">
        ...
        <Section title="Personal Details" spacing="sm">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <TextArea label="Address" rows={2} {...register('address')} className="sm:col-span-2" />
          </div>
        </Section>

        <Section title="Guardian Details" spacing="sm">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
