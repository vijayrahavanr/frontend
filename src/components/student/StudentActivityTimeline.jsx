import { FiCheckCircle, FiCalendar, FiCamera, FiUserCheck, FiBell } from 'react-icons/fi';
...
const TYPE_CONFIG = {
  ...
  qr: { icon: FiCamera, className: 'bg-primary-50 text-primary dark:bg-primary-900/20' },
  face: { icon: FiUserCheck, className: 'bg-secondary-50 text-secondary dark:bg-secondary-900/20' },
  notification: { icon: FiBell, className: 'bg-slate-100 text-slate-500 dark:bg-slate-800' },
};

/**
 * Vertical timeline of recent student activity (attendance marked,
 * leave approved, QR scan, face verification, etc). Shown on the
 * Dashboard.
 *
 * @param {object} props
 * @param {{id: string|number, type: keyof typeof TYPE_CONFIG, title: string, timestamp: string}[]} props.activities
 */
const StudentActivityTimeline = ({ activities = [], className }) => (
