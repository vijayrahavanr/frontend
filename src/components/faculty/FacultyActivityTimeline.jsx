import { FiCheckCircle, FiCalendar, FiUserCheck, FiBell, FiFileText } from 'react-icons/fi';
import { formatRelativeTime } from '@/utils/date.utils';
import { cn } from '@/utils/helpers';

const TYPE_CONFIG = {
  attendance: { icon: FiCheckCircle, className: 'bg-success/10 text-success' },
  leave: { icon: FiCalendar, className: 'bg-warning/10 text-warning' },
  student: { icon: FiUserCheck, className: 'bg-secondary-50 text-secondary dark:bg-secondary-900/20' },
  report: { icon: FiFileText, className: 'bg-primary-50 text-primary dark:bg-primary-900/20' },
  notification: { icon: FiBell, className: 'bg-slate-100 text-slate-500 dark:bg-slate-800' },
};

/**
 * Vertical timeline of recent faculty activity (attendance marked,
 * leave reviewed, report generated, etc). Shown on the Dashboard.
 *
 * @param {object} props
 * @param {{id: string|number, type: keyof typeof TYPE_CONFIG, title: string, timestamp: string}[]} props.activities
 */
const FacultyActivityTimeline = ({ activities = [], className }) => (
...
export default FacultyActivityTimeline;
